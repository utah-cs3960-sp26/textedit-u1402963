...
# How to run
Open in VScode, click the run button. There is a launch.json file in the .vscode folder. Testing should also work in the same way, just click on the VScode test section and click run. 

# How to run (CLI)
To run : `python src/main.py`
Run tests: `pytest`, windows: `python -m pytest`
Install dependencies (if needed): `pip install PyQt6 pytest`
