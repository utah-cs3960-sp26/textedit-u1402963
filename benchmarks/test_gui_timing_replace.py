...
class TestGuiTimingReplace:
    ...
    def _run_replace_with_timeout(self, window, qapp, run_timed_with_timeout, content, label):
        find_bar = window._find_replace_bar
        find_bar.show_bar()
        find_bar._find_input.setText("while")
        find_bar._replace_input.setText("for")
        ...
        passed = run_timed_with_timeout(label, lambda: find_bar.replace_all(), setup=setup)

        find_bar.hide_bar()
        qapp.processEvents()
        return passed

    def test_gui_timing_replace_large(self, window, qapp, run_timed_with_timeout, large_content, require_small_medium_pass):
        passed = self._run_replace_with_timeout(
            window, qapp, run_timed_with_timeout, large_content,
